...
export function About({ profile, skills }: { profile: Profile | null; skills: Skill[] }) {
  ...
  return (
    <section id="about" className="mx-auto max-w-6xl px-4 py-20 sm:px-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="card-surface min-w-0 p-6 lg:col-span-2">
        ...
        <div className="grid gap-6">
          <div className="card-surface p-6">
            <h3 className="flex items-center gap-2 font-semibold">
              <Sparkles className="h-4 w-4 text-accent" /> Passion & Currently Doing
            </h3>
...
export function Academic({ entries, skills }: { entries: AcademicEntry[]; skills: Skill[] }) {
  const technical = skills.filter((s) => s.category === "Technical");
  return (
    <section id="academic" className="border-y border-border bg-secondary/40 py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <h3 className="mt-14 mb-5 text-xl font-bold tracking-tight">Technical Skills</h3>
...
export function StandUp({ profile, skills }: { profile: Profile | null; skills: Skill[] }) {
  const highlights = skills
    .filter((s) => s.category !== "Technical")
    .slice(0, 6)
    .map((s) => s.name);
  return (
    <section id="standup" className="relative overflow-hidden py-20">
      ...
      <div className="relative mx-auto max-w-6xl px-4 text-primary-foreground sm:px-6">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="min-w-0">
            ...
            <div className="mt-6 inline-flex items-center gap-3 rounded-xl border border-accent/50 bg-background/10 px-4 py-3">
              <Trophy className="h-5 w-5 shrink-0 text-accent" />
              <span className="text-sm font-semibold">Champion in Stand-Up Comedy — UIU</span>
            </div>
          ...
          <div className="min-w-0 rounded-xl border border-primary-foreground/20 bg-background/10 p-6 backdrop-blur-sm">
...
export function Achievements({
  ...
  return (
    <section id="achievement" className="mx-auto max-w-6xl px-4 py-20 sm:px-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="card-surface min-w-0 p-6">
          <h3 className="flex items-center gap-2 text-lg font-semibold">
            <Star className="h-5 w-5 text-accent" /> Truss Competition Participation
          </h3>
...
export function Gallery({ photos }: { photos: GalleryPhoto[] }) {
  return (
    <section id="gallery" className="border-y border-border bg-secondary/40 py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <div className="columns-1 gap-4 sm:columns-2 lg:columns-3 [&>*]:mb-4">
            {photos.map((p) => (
              ...
              >
                ...
                />
                {(p.title || p.caption || p.featured) && (
                  <figcaption className="p-4">
                    {p.featured ? (
                      <span className="mb-1 inline-block rounded-full bg-accent/20 px-2 py-0.5 text-[11px] font-semibold text-accent-foreground">
                        Featured
                      </span>
                    ) : null}
    ...
    <section id="contact" className="mx-auto max-w-6xl px-4 py-20 sm:px-6">
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        {profile.content_page_url ? (
          <Button asChild variant="outline">
            <a href={profile.content_page_url} target="_blank" rel="noreferrer noopener">
              <CalendarClock className="h-4 w-4" /> {profile.content_page_name}
            </a>
          </Button>
        ) : null}
      </div>
